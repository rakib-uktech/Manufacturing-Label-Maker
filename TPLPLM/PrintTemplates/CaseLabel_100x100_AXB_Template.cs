...
public class CaseLabel_100x100_AXB_Template
{
    public void Print(
        LabelInfo labelinfo,
        ...
        long labelNoBase)
    {
        ...
        {
            throw new Exception("AXB Template Error: " + ex.Message);
        }
    }
    ...
    // BOTTOM BLOCK METHOD
    // ======================
    private void DrawBottomBlock(PrintPageEventArgs e, ref float y, float x,
        LabelInfo labelinfo, AssemblyItem item, string labelNo)
    {
        ...
        void DrawPair(string label, string value)
        {
            ...
            currentY += 14;
            e.Graphics.DrawString(value, bold, Brushes.Black, new PointF(x, currentY));
            currentY += 18;
        }

        DrawPair("Date Produced:", labelinfo.Create_Date);
        DrawPair("Our Ref:", item?.ItemId ?? "N/A");
        DrawPair("Case Number:", labelNo);

        float bottomY = currentY;

        using var pen2 = new Pen(Color.Black, 2);
        e.Graphics.DrawLine(pen2, x, bottomY, x + tableWidth, bottomY);
