...
{
    public class PalletLabel_150x200_Daylesford_Template
    {
        public void Print(
            LabelInfo labelinfo,
            ...
            long labelNoBase)
        {
            try
            {
                ...
                for (int i = 0; i < printCount; i++)
                {
                    ...
                    if (!printerSettings.IsValid)
                        throw new Exception(
                            $"Selected printer '{SelectedPrinter}' is not valid.");
                    ...
                    var highRes = printerSettings.PrinterResolutions
                        ...
                        .FirstOrDefault(r =>
                            r.Kind == PrinterResolutionKind.High);
                    ...
                    var forcedSize =
                        new PaperSize("Forced6x8", 600, 800);
                    ...
                    printDoc.PrintPage += (sender, e) =>
                    {
                        ...
                        if (System.IO.File.Exists(imagePath))
                        {
                            ...
                            float logoRatio = img.Height / (float)img.Width;
                            int logoWidth = 200;
                            int logoHeight = (int)(logoWidth * logoRatio);
                            ...
                            encodingOptions.Hints[ZXing.EncodeHintType.DATA_MATRIX_SHAPE] = ZXing.Datamatrix.Encoder.SymbolShapeHint.FORCE_SQUARE;
                            ...
                            SizeF originValueSize = e.Graphics.MeasureString(originValue, valueFont);

                            float totalTextHeight = labelSize.Height + originLabelSize.Height + 2;
                            float maxBlockHeight = Math.Max(logoHeight, Math.Max(barcodeSize, totalTextHeight));

                            // Define X positions
                            float logoX = x;
                            float barcodeX = x + logoWidth + 40; // center-ish
                            float textX = barcodeX + barcodeSize + 40; // right side

                            // Vertically align
                            float topY = y + (maxBlockHeight - logoHeight) / 2f;
                            float barcodeY = y + (maxBlockHeight - barcodeSize) / 2f;
                            float textY = y + (maxBlockHeight - totalTextHeight) / 2f;

                            // Draw logo
                            e.Graphics.DrawImage(img, new RectangleF(logoX, topY, logoWidth, logoHeight));

                            // Draw barcode center
                            e.Graphics.DrawImage(matrixBitmap, new RectangleF(barcodeX, barcodeY, barcodeSize, barcodeSize));

                            // Draw right-side text (TPL Ref and Origin)
                            e.Graphics.DrawString(label, labelFont, Brushes.Black, new PointF(textX, textY));
                            e.Graphics.DrawString("", valueFont, Brushes.Black, new PointF(textX + labelSize.Width + 5, textY));

                            float originY = textY + labelSize.Height + 2;
                            e.Graphics.DrawString(originLabel, labelFont, Brushes.Black, new PointF(textX, originY));
                            e.Graphics.DrawString(originValue, valueFont, Brushes.Black, new PointF(textX + originLabelSize.Width + 5, originY));
                        ...
                        {
                            e.Graphics.DrawString(Address, new Font("Arial", 11), Brushes.Black, new PointF(x, y));
                            y += 25;
                        ...
                        PrintInline(
                           "Product Code:", AssemblyItem?.ItemId ?? "N/A",
                        ...
                        //PrintInline("Date Produced:", labelinfo.Create_Date, "", "");
                        PrintInline(
                        ...
                        string palletGtinValueWithDate =
                              gtinValue + prodDateValue + caseNoValue + palletWeightValue;
                        ...
                        string readableGtinWithDate = readablePalletGtin + readableProdDate + readableCaseNo + readablePalletWeight;
            ...
            {
                throw new Exception(
                    "Daylesford Pallet Label Error: " + ex.Message);
            }
        }
