...
public class CaseLabel_100x100_Daylesford_Template
{
    public void Print(
        LabelInfo labelinfo,
        ...
        long labelNoBase)
    {
        try
        {
            ...
            for (int i = 0; i < printCount; i++)
            {
                ...
                printDoc.PrintPage += (sender, e) =>
                {
                    ...
                    void PrintPacksizeRowInline(string label1, string value1, string label2, string value2)
                    {
                        var boldFont = new Font("Arial", 11, FontStyle.Bold);
                        float colSpacing = 250;
                        ...
                        e.Graphics.DrawString(label1, font, Brushes.Black, new PointF(x, y));
                        e.Graphics.DrawString(value1, boldFont, Brushes.Black, new PointF(x + 110, y));
                        e.Graphics.DrawString(label2, font, Brushes.Black, new PointF(x + 130, y));
                        e.Graphics.DrawString(value2, boldFont, Brushes.Black, new PointF(x + 140, y));
                    ...
                    // Inline case row
                    PrintPacksizeRowInline(
                         "Pack Size:",
                         decimal.TryParse(
                             AssemblyItem?.Custitemproduct_Spec_Qtyperinnerlayer,
                             out var size)
                                 ? Math.Round(size).ToString("0")
                                 : "Error!",

                         "X",

                         decimal.TryParse(
                             AssemblyItem?.Custitemproduct_Spec_Innlaypercase,
                             out var innerCase)
                                 ? Math.Round(innerCase).ToString("0")
                                 : "Error!"
                     );
                    // Inline case row
                    PrintCaseRowInline(
                        ...
                        string caseLabel, string caseValue)
                    {
                        ...
                        if (System.IO.File.Exists(imagePath))
                        {
                            ...
                            float imageRatio =
                                (float)papImage.Width / papImage.Height;
                        ...
                        float poLabelWidth =
                            e.Graphics.MeasureString(poLabel, regularFont).Width;
                        ...
                        float maxLabelWidth = Math.Max(
                            Math.Max(dateLabelWidth, poLabelWidth),
                            Math.Max(refLabelWidth, caseLabelWidth));

                        float labelAreaWidth = maxLabelWidth + linePadding;

                        float valueAreaWidth =
                            tableWidth - labelAreaWidth - padding * 2;
                        ...
                        if (papImage != null)
                        {
                            float threeRowsHeight = (rowHeight * 3) + (rowSpacing * 2);

                            float imageX =
                                valueX + valueAreaWidth - imageWidth - 15f;

                            float imageY =
                                y + (threeRowsHeight - imageHeight) / 2f;
                    ...
                    PrintDateWithProductCodeAndRefTable(
                        ...
                        "PO Number:", labelinfo?.Work_Order ?? "N/A",
        ...
        {
            throw new Exception(
                "Daylesford Template Print Error: " + ex.Message);
        }
    }
}